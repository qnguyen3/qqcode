...
async def test_respects_vibeignore_file(grep, tmp_path):
    (tmp_path / ".qqcodeignore").write_text("custom_dir/\n*.tmp\n")
...
async def test_ignores_comments_in_vibeignore(grep, tmp_path):
    (tmp_path / ".qqcodeignore").write_text("# comment\npattern/\n# another comment\n")
...
class TestGnuGrepBackend:
    ...
    @pytest.mark.asyncio
    async def test_respects_qqcodeignore_file(self, grep_gnu_only, tmp_path):
        (tmp_path / ".qqcodeignore").write_text("custom_dir/\n*.tmp\n")
        custom_dir = tmp_path / "custom_dir"
        custom_dir.mkdir()
        (custom_dir / "excluded.py").write_text("match\n")
