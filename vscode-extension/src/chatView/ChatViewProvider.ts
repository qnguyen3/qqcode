...
export class ChatViewProvider implements vscode.WebviewViewProvider {
    ...
    private saveState(): void {
        this.extensionContext.workspaceState.update('qqcode.currentSessionId', this.state.currentSessionId);
        this.extensionContext.workspaceState.update('qqcode.messageHistory', this.state.messageHistory);
    }

    private restoreState(): void {
        this.state.currentSessionId = this.extensionContext.workspaceState.get<string | null>('qqcode.currentSessionId', null);
        this.state.messageHistory = this.extensionContext.workspaceState.get<Array<{ role: string; content: string }>>('qqcode.messageHistory', []);
        ...
        const savedMode = this.extensionContext.globalState.get<ExecutionMode>('qqcode.selectedMode');
        if (savedMode) {
    ...
    resolveWebviewView(
        ...
        webviewView.webview.onDidReceiveMessage(
            (data: WebviewToExtensionMessage) => this.handleWebviewMessage(data)
        );
    ...
    private async handleUserMessage(text: string): Promise<void> {
        ...
        try {
            ...
            this.state.messageHistory.push({ role: 'assistant', content: assistantMessage });
    ...
    private handleSessionStarted(returnedSessionId: string, requestedSessionId: string | null): void {
        if (!this.state.currentSessionId) {
            this.state.currentSessionId = returnedSessionId;
            console.log(`[QQCode] New session started: ${this.state.currentSessionId}`);
            ...
            console.log(`[QQCode] Continuing session: ${this.state.currentSessionId}`);
        }
                ...
                if (sessionData) {
                    for (const msg of sessionData.messages) {
                        if ((msg.role === 'user' || msg.role === 'assistant') && msg.content) {
                            ...
                            this.state.messageHistory.push({ role: msg.role, content: msg.content });
                        }
                    }
                }
            } catch (error) {
